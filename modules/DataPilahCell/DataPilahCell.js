...
(function () {
    ...
    MyApp.$me.draw = 0;
    MyApp.$me.aksi = 'add';
    var pilihKode = $('.pilihKode');
    var pilihKodeKolom = $('.pilihKodeKolom');
    var pilihKodeBaris = $('.pilihKodeBaris');
    ...
    MyApp.ajax(paramKode).done(function (resp) {
        pilihKode.html("<option value=''>--Pilih Kode Data Pilah--</option>");
        $.each(resp.result, function (index, val) {
            pilihKode.append('<option value="' + val.kode_data_pilah + '">' + val.kode_data_pilah + " : " + val.judul_data_pilah + '</option>');
        });
        pilihKode.select2();
    })

    
    pilihKodeKolom.html("<option value=''>--Pilih Kode Kolom--</option>");
    pilihKodeKolom.select2();
    pilihKode.on("change", function () {
        kodeDataPilah=pilihKode.val();

        var paramKodeKolom = {
            option: "ACTION",
            action: "getKodeKolom",
            kode_data_pilah: kodeDataPilah
        }
        MyApp.ajax(paramKodeKolom).done(function (resp) {
            pilihKodeKolom.html("<option value=''>--Pilih Kode Kolom--</option>");
            $.each(resp.result, function (index, val) {
                pilihKodeKolom.append('<option value="' + val.kode_kolom + '">' + val.kode_kolom + '</option>');
                if(kodeKolom != ''){
                    $(".pilihKodeKolom").select2("val", kodeKolom);
                }
                
            });
            pilihKodeKolom.select2();
            // $(".cbKelurahan").select2("val", idKel);
        })
    })

    pilihKodeBaris.html("<option value=''>--Pilih Kode Baris--</option>");
    pilihKodeBaris.select2();
    pilihKode.on("change", function () {
        kodeDataPilah=pilihKode.val();
        ...
        MyApp.ajax(paramKodeBaris).done(function (resp) {
            pilihKodeBaris.html("<option value=''>--Pilih Kode Baris--</option>");
            $.each(resp.result, function (index, val) {
                pilihKodeBaris.append('<option value="' + val.kode_baris + '">' + val.kode_baris + '</option>');
                if(kodeBaris != ''){
                    $(".pilihKodeBaris").select2("val", kodeBaris);
                }
            });
            pilihKodeBaris.select2();
        })
    ...
    MyApp.$me.selectorTable = $('#datatable_fixed_column').DataTable({
        ...
        "columns": [
            ...
            {"data": "tahun"},{"data": "kode_data_pilah"},{"data": "kode_kolom"},{"data": "kode_baris"},{"data": "val"},
        ],
        dom: 'Bfrtip',
        ...
        initComplete: function () {
            ...
            $('.btTop').click(function () {
                ...
                if(aksi === 'add'){
                    ...
                    $('.modal-title').html("Tambah Data Pilah Cell");
            ...
            selectortable.on('click','.btUpdate',function () {
                ...
                console.log(dataRow);
                $('.modal-title').html("Update Data Pilah Cell");
                kodePilah=dataRow.kode_data_pilah;
                $(".pilihKode").select2("val", kodePilah);
                kodeKolom=dataRow.kode_kolom;
                $(".pilihKodeKolom").select2("val", kodeKolom);
                kodeBaris=dataRow.kode_baris;
                $(".pilihKodeBaris").select2("val", kodeBaris);
                dataRow = MyApp.$me.dataRow[index];
            ...
            selectortable.on('click','.btHapus',function () {
                index = $(this).data('index');
                dataRow = MyApp.$me.dataRow[index];
                konfirmasi = confirm('Apakah anda yakin ingin menghapus data ini?');
                if(konfirmasi){
                    hapus(dataRow);
                }
            });
            ...
            if (resp.success) {
                MyApp.loadModuleFile('template/tpl_pdf.html', function (tpl) {
                    ...
                    for (i=0;i<resp.result.length;i++){
                        data2[i]={};data2[i].id_data_pilah = nama=resp.result[i].id_data_pilah;data2[i].judul_data_pilah = nama=resp.result[i].judul_data_pilah;data2[i].kode_data_pilah = nama=resp.result[i].kode_data_pilah;data2[i].aktif = nama=resp.result[i].aktif;data2[i].kode_instansi = nama=resp.result[i].kode_instansi;data2[i].instansi = nama=resp.result[i].instansi;data2[i].header_baris = nama=resp.result[i].header_baris;
                    }
    ...
    $('.btSimpan').on('click', function () {
        if(pilihKode.val()=="" || pilihKodeKolom.val()=="" || pilihKodeBaris.val()=="") {
            alert("Kode data pilah, kode kolom, dan kode baris harus diisi !")
        } else {
            idForm = $('#form_input');

...
class DataPilahBaris extends Database
{
    ...
    private function findField() {
        $findField = [
            "kode_data_pilah","no_urut","kode_baris","nama_baris","aktif"
        ];
        return $findField;
    }
    ...
    public function ACTION_list($return=false)
    {
        ...
        $sql = 'SELECT * FROM data_pilah_baris';
        ...
        $array = array();
        $sqlCount = "SELECT count(*) FROM data_pilah_baris";
    ...
    public function ACTION_add(){
        $params = isset($_GET) ? $_GET : $_POST;
        $kode_baris = $params['data']['kode_baris'];
        $sql_k = "SELECT * FROM data_pilah_baris WHERE kode_baris=:kode_baris";
        $sql = "INSERT INTO `data_pilah_baris`(`id_data_pilah_baris`, `kode_data_pilah`, `no_urut`, `kode_baris`, `nama_baris`, `aktif`) 
                VALUES (NULL,:kode_data_pilah,:no_urut,:kode_baris,:nama_baris,:aktif)";
        
        $checkParams = ['kode_baris' => $kode_baris];
    ...
    public function ACTION_update(){
        ...
        $kode_baris = $params['data']['kode_baris'];
        $sql_k = "SELECT * FROM data_pilah_baris WHERE kode_baris=:kode_baris AND id_data_pilah_baris != :id";
        $sql = "update data_pilah_baris set kode_data_pilah=:kode_data_pilah,no_urut=:no_urut,kode_baris=:kode_baris,nama_baris=:nama_baris,aktif=:aktif
                 WHERE id_data_pilah_baris=:id_data_pilah_baris";
        
        $checkParams = ['kode_baris' => $kode_baris, 'id' => $id];
        if($this->dbDataRowsCount($sql_k, $checkParams) > 0) {
            echo '{"success" : false,"msg": "Kode Baris Sudah Ada"}';
        }else {
            echo $this->dbDataExecute($sql,$params['data']);
    ...
    public function ACTION_delete(){
        $params = isset($_GET) ? $_GET : $_POST;
        $sql = "delete from data_pilah_baris WHERE id_data_pilah_baris=:id_data_pilah_baris";
        echo $this->dbDataExecute($sql,$params['data']);
    }

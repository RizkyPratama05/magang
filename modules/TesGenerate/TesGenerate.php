...
class TesGenerate extends Database
{
    ...
    private function findField() {
        $findField = [
            "action_id","option","action","description","log"
        ];
        return $findField;
    }
    ...
    public function ACTION_list($return=false)
    {
        ...
        $array = array();
        $sqlCount = "SELECT count(*) FROM actions ";
    ...
    public function ACTION_add(){
        $params = isset($_GET) ? $_GET : $_POST;
        $sql = "insert into actions (action_id,option,action,description,log) VALUES 
                                          (:action_id,:option,:action,:description,:log)";
    ...
    public function ACTION_update(){
        $params = isset($_GET) ? $_GET : $_POST;
        $sql = "update actions set action_id=:action_id,option=:option,action=:action,description=:description,log=:log
    ...
    public function ACTION_delete(){
        $params = isset($_GET) ? $_GET : $_POST;
        $sql = "delete from actions WHERE module_id=:module_id";
        echo $this->dbDataExecute($sql,$params['data']);
    }

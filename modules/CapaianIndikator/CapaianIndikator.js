...
(function () {
    ...
    var cbUrusan = $('.cbUrusan');
    var fieldIndikator = $('.kodeIndikator');
    ...
    MyApp.ajax(paramUrusan).done(function (resp) {
        cbUrusan.append("<option value='0'> --Pilih Urusan-- </option>");
        $.each(resp.result, function (index, val) {
            cbUrusan.append("<option value='" + val.kode_indikator + "'>" + val.kode_indikator + " - " + val.indikator + "</option>");
        })
        cbUrusan.select2();
        // debugger;
    })
    ...
    /*on change cbUrusan*/
    cbUrusan.on('change', function () {
        val = $(this).val();
        if (val == 0) {
            fieldIndikator.val('');
        } else {
            fieldIndikator.val(val + ".");
        }
        $('.kodeIndikator').focus();
    })
    ...
    MyApp.$me.selectorTableCapaian = $('#datatable_fixed_column').DataTable({
        serverSide: true,
        processing: true,
        destroy: true,
        bInfo: true,
        ...
        "columns": [
            ...
            },
            {
                "data": function (val, item, index) {
                    MyApp.$me.dataRow[val.id_capaian] = val;
                    btn = "<a style='padding: 2px;' href='#' class='btUpdate' data-index='" + val.id_capaian + "'" +
                        " title='Update data'><i class='fa fa-pencil-square fa-2x'></i></a>" +
                        "<a style='color:#e80531;padding: 2px;' href='#' data-index='" + val.id_capaian + "' " +
                        "title='Hapus Data' class='btHapus'>" +
                        "<i class='fa fa-trash-o fa-2x'></i></a>";
                    level = val.kode_indikator.split('.').length;
                    // debugger;
                    if (level != 1) {
                        btn += "<a style='color:#00a65a;padding: 2px;' href='#' data-index='" + val.id_capaian + "' title='Tambah Rumus' class='btRumus'>" +
                            "<i class='fa fa-qrcode fa-2x'></i></a>" +
                            "<a style='padding: 2px;color: #AA7639' href='#' data-index='" + val.id_capaian + "' title='Lihat detail Data' class='btLihatData '>" +
            ...
            {"data": "kode_indikator", width: '10%'},
            {"data": "indikator", width: '25%'},
            {"data": "satuan"},
            {"data": "t2013"},
            {"data": "t2014"},
            {"data": "t2015"},
            ...
            {"data": "t2017"},
            {
                "data": function (val, item) {
                    level = val.kode_indikator.split('.').length;
                    if (level != 1 && val.canvas && val.canvas != "data:,") {
                        rumus = "<img style='width:150px;' src='" + val.canvas + "'><br><ul style='margin-left: -25px;font-size: 10px;'>";
                        $.each(val.sumberdata,function (index, val) {
                            rumus +="<li><b>"+val.simbol_temp+" = </b>"+val.sumberdata+"</li>";
                        })
                        rumus+="</ul>"
                    } else {
                        rumus = '';
        ...
        rowCallback: function (row, data, index) {
            /*debugger;
            if (data.level != 1 && data.canvas2 && data.canvas2 != "data:,") {
                rumus = "<img src='" + data.canvas2 + "'>";
            } else {
                rumus = '';
            }
            $('td:eq(4)', row).html(rumus);*/
        ...
        initComplete: function () {
            ...
            button = '<button data-aksi="refresh" style="margin: 5px 5px -40px 0" class="btn btn-danger btTop">' +
                '<i class="fa fa-refresh"></i> <span> Refresh</span>' +
                '</button>' +
                '<button data-aksi="add" style="margin: 5px 5px -40px 0" class="btn btn-success btTop">' +
                '<i class="fa fa-plus"></i> <span> Tambah Data</span>' +
                '</button>' +
                '<button data-aksi="pdf" style="margin: 5px 5px -40px 0" class="btn btn-warning btTop">' +
                '<i class="fa fa-file-pdf-o"></i> <span> PDF</span>' +
                '</button>' +
                /*'<button data-aksi="excel" style="margin: 0 5px 0 0" class="btn btn-app btTop">' +
                 '<i class="fa fa-file-excel-o"></i> <span> Excel</span>' +
                 '</button>'+*/
                '<button data-aksi="print" style="margin: 5px 5px -40px 0" class="btn btn-primary btTop">' +
            ...
            $('.btTop').click(function () {
                ...
                if (aksi === 'add') {
                    ...
                    MyApp.$me.selectorTableCapaian.ajax.reload();
                } else if (aksi === 'pdf') {
                    export_pdf();
                } else if (aksi === 'print') {
                    print_hal();
            ...
            /*Tambah rumus*/
            selectortable.on('click', '.btRumus', function () {
                index = $(this).data('index');
                dataRow = MyApp.$me.dataRow[index];
                MyApp.openModuleWindow('hitung-rumus', dataRow)
            });
    ...
    var loadViewData = function(params){
        MyApp.$me.selectorTableBulan = $('#tabel_data_bulan').DataTable({
            serverSide: false,
            processing: true,
            destroy: true,
            paging:false,
            ...
            ajax: {
                ...
                data: {
                    ...
                    id_capaian_indikator : params.id_capaian,
                    kode_indikator : params.kode_indikator,
                    draw: function () {
                        MyApp.$me.draw++;
                        return MyApp.$me.draw;
                    }
            ...
            if (resp.success) {
                MyApp.loadModuleFile('template/tpl_pdf.html', function (tpl) {
                    ...
                    for (i = 0; i < resp.result.length; i++) {
                        data2[i] = {};
                        data2[i].id_capaian = nama = resp.result[i].id_capaian;
                        data2[i].indikator = nama = resp.result[i].indikator;
                        data2[i].id_urusan = nama = resp.result[i].id_urusan;
                        data2[i].nilai_data = nama = resp.result[i].nilai_data;
                        data2[i].keterangan = nama = resp.result[i].keterangan;
                        data2[i].canvas = nama = resp.result[i].canvas;
                        ...
                        data2[i].user_input = nama = resp.result[i].user_input;
                        data2[i].operasi_math_query = nama = resp.result[i].operasi_math_query;
                        data2[i].operasi_math = nama = resp.result[i].operasi_math;
                        data2[i].json_rumus = nama = resp.result[i].json_rumus;

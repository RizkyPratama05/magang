...
class SkpeHasIndikator extends Database
{
    ...
    public function ACTION_list($return=false)
    {
        ...
        $sql = 'SELECT * FROM ref_indikator';
        if(isset($_POST['search']['value']) && $_POST['search']['value'] !=''){
            $keywords = strtolower($_POST['search']['value']);
            $findField = $this->findField();
            $criteria = $this->buildSqlSearchingCriteria($keywords, $findField);
            $sql .= " where ".$criteria;
            $sql.=" and kode_indikator not in(select kode_indikator from instansi_has_indikator where kode_instansi='$kodeInstansi')";
        }else{
            $sql.=" where kode_indikator not in(select kode_indikator from instansi_has_indikator where kode_instansi='$kodeInstansi')";
...
//        echo $sql;exit();
    ...
    public function ACTION_list2($return=false)
    {
        ...
        if(isset($_POST['search']['value']) && $_POST['search']['value'] !=''){
            ...
            $criteria = $this->buildSqlSearchingCriteria($keywords, $findField);
            $sql .= " where ".$criteria;
            $sql.=" and kode_indikator in(select kode_indikator from instansi_has_indikator where kode_instansi='$kodeInstansi')";
        }else{
            $sql.=" where kode_indikator in(select kode_indikator from instansi_has_indikator where kode_instansi='$kodeInstansi')";
...
//        echo $sql;exit();
    ...
    public function ACTION_tambahkan(){
        $params = isset($_GET) ? $_GET : $_POST;
        $sql = "insert into instansi_has_indikator (kode_instansi,kode_indikator) VALUES 
                                          (:kode_instansi,:kode_indikator)";
    ...
    public function ACTION_delete(){
        $params = isset($_GET) ? $_GET : $_POST;
        $sql = "delete from instansi_has_indikator WHERE kode_instansi=:kode_instansi AND kode_indikator=:kode_indikator";
        echo $this->dbDataExecute($sql,$params['data']);
    }
...
//        $userUnit = $this->ACTION_getUserInstansi(true);
        $urlApi = 'http://apis.slemankab.go.id/try.php?username=sso&api_key=fa531adb3f816c0c57ee9c5e50ce04af&app=teppa&module=getListInstansi&action=read&json={%22tahun%22:%22'.$currYear.'%22}';
        $response = file_get_contents($urlApi);
//        $response = file_get_contents('http://teppa.slemankab.go.id/_i.php/getListInstansi/{"tahun":'.$currYear.'}');
        //$urlPar = urlencode('{"tahun":'.$currYear.'}');
        //$url = urlencode('https://esb.slemankab.go.id:8246/teppa/listintansi/v.1/{"tahun":'.$currYear.'}');
        //echo $url; exit;
        //$response = file_get_contents('https://esb.slemankab.go.id:8246/teppa/listintansi/v.1/'.$urlPar);
